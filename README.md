<div align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/version-4.8.0-00AEEC?style=flat-square">
...
齿轮图标用于修改 API、模型与提示词；刷新图标用于基于同一份字幕重新生成总结。设置窗口提供通用总结、要点提纲、学习笔记、行动清单和视频叙事五种提示词模板，选择后仍可继续编辑。

| 快捷键 | 操作 |
| --- | --- |
| `Alt + Shift + A` | 打开或触发「AI 总结」。 |
| `Esc` | 收起已打开的总结面板。 |
...
| --- | --- |
| 字幕获取 | 读取播放器提供的可用字幕。 |
| 字幕优先级 | 可选择 AI 中文字幕、任意 AI 字幕、原始字幕或任意可用字幕作为优先来源。 |
| 字幕复制 | 点击“字幕 xx 行”复制原始字幕文本。 |
| AI 总结 | 根据字幕与提示词生成内容摘要；生成中断时保留已经返回的内容。 |
| 结果导出 | 支持复制总结内容或下载 Markdown 文件。 |
| 提示词模板 | 提供通用总结、要点提纲、学习笔记、行动清单和视频叙事模板。 |
| 键盘快捷键 | 支持快速打开总结面板与收起面板。 |
| 本地总结历史 | 按视频保存最近 20 条总结；再次打开时可直接查看或清除已保存内容。 |
| 深色模式 | 跟随系统深色偏好调整总结面板与设置窗口。 |
| 关键节点跳转 | 总结会列出重要片段；点击 `[MM:SS]` 或 `[HH:MM:SS]` 可跳转至视频对应位置。 |
| 自定义提示词 | 支持摘要、笔记、问答、行动清单和叙事分析等表达形式。 |
